 ...
 * 所以我们现在的目标是寻找一个Runnable的实现类target，它能在run结束后保存结果。而且我们需要创造一个接口 此接口能够返回result
 * 并且此接口可以让用户自定义内容
 * <p>
 * 所以我们创造了 Callable 此接口的call可以返回result 而我们可以将逻辑代码放入其中
 * FutureTask 为runnable的实现类，此类需要一个Callable实现类获取result，run方法会调用call并且保存结果
 * <p>
 * 流程：
 ...
 * public void run() 执行Callable 但是执行任务的只能有一个线程，其他的线程会直接返回.当执行完成后会唤醒 因调用get而阻塞的线程
 * 总体来说 Callable代替 Runnable实现逻辑代码，而FutureTask包装Callable 适应Thread，Future接口去获取结果
