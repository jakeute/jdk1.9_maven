...
public class StringTest {
    ...
    public void StringTest_init() {
        ...
        // 新版本是byte[]  char是两个字节，如果用来存一个字节的字符编号有点浪费，为了节约空间，jdk就改成了一个字节的byte来存储字符串。
        //这样在存储一个字节的字符是就避免了浪费 String中维护了一个新的属性 coder，它是编码格式的标识，在计算字符串长度或者调用 indexOf () 函数时，
        //需要根据这个字段，判断如何计算字符串长度。coder 属性默认有 0 和 1 两个值，0 代表Latin - 1（单字节编码），1 代表双字节编码编码。
    ...
    public void StringTest_usual() {
        ...
        //replaceFirst() 根据正则表达式替换第一个
        //replaceAll() 根据正则表达式替换所有
